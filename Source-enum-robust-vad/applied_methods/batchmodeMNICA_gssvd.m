% Implementation of voice activity detection by GSMM-NICA from [1]
%
%     Copyright (C) 2017 Lala Khadidja Hamaidi, for the method in [2] 
%     (Modified and adjusted to [1] by Martin Gölz)  
%
...
%[2] L. K. Hamaidi, M. Muma, and A. M. Zoubir, “Multi-speaker voice activity
...
function [S]=batchmodeMNICA_gssvd(Y,Q,nbiter,N_j)
    
    %Making sure that we use only complete blocks by truncating the input
    %signal if necessary
    N = size(Y,2);
    Y = Y(:,1:N_j*floor(N/N_j));
    
    %The sparse singular value decomposition
    [Y,U,V,SIGMA]=Reconstruction_gssvd(Y,Q,N_j);
    
    %All the remaining code was left untouched (provided by Khadidja)
    V1=V;
